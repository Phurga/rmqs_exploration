...
def compute_cf_median_classified_references(
        data: pd.DataFrame, 
        classifiers = ["bioregion", 'WRB_LVL1'],
    ...
    data[f"reference_median_{indicator}"] = data.apply(
        lookup_reference, 
        args=(reference_land_use, median_indicator_context, classifiers, indicator), 
        axis=1)
    ...
    for classifier in classifiers:
        plot_distribution(data, f"relative_{indicator}", classifier, "soil_type")
